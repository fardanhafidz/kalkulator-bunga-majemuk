...
const waktuSetoranInput = document.getElementById('waktuSetoran');
...
function calculateAndVisualize() {
    ...
    for (let bulan = 1; bulan <= totalBulan; bulan++) {
        ...
        // Logic Awal Bulan: Setor masuk sebelum bunga dihitung
        if (isAwalBulan) {
        ...
        // 2. Update saldo dengan bunga
        saldo += bungaBulanIni;

        // Logic Akhir Bulan: Setor masuk setelah bunga dihitung (tidak berbunga bulan ini)
        if (!isAwalBulan) {
            saldo += cicilanBulanan;
            totalUangDisetor += cicilanBulanan;
        }
